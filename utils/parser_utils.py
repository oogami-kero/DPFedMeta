...
def get_args():
    ...
    parser.add_argument('--clip_norm_init', type=float, default=40.0)
    parser.add_argument('--adaptive_clip', type=str, default="true")
    ...
    parser.add_argument('--experiment_type', type=str, default="meta_initial")
    parser.add_argument('--inner_z', type=int, default=1, help="inner loop noise multiplier")
    parser.add_argument('--window_size', type=int, default=1, help="slide window size, a multiplier of lot size")
    ...
    if args.use_cuda:
        ...
        device = torch.device('cuda:' + str(args.gpu_to_use))
    else:
